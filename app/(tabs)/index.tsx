    ...
    <View style={styles.sectionBlock}>
      ...
      <View style={styles.budgetBarBg}>
        <LinearGradient
          ...
          style={[styles.budgetBarFill, { width: `${Math.min(percent, 100)}%` }]}
        />
      </View>
...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    // Redirect to survey if connected/demo but survey not done
    if ((isConnected || isDemoMode) && !isLoading && !isSurveyCompleted) {
      router.replace("/onboarding-survey");
    }
  }, [isConnected, isDemoMode, isLoading, isSurveyCompleted]);
